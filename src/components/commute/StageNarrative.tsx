...
export function StageNarrative({ progress, perspective = 'sender' }: { progress: number; perspective?: Perspective }) {
  ...
  return <section className="stage-narrative" aria-live="polite">
    <AnimatePresence mode="sync" initial={false}>
      <motion.div className="narrative-copy" key={`${perspective}-${stage.id}-${passage.at}`} initial={{ opacity: .25, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .32, ease: [0.22, 1, 0.36, 1] }}>
